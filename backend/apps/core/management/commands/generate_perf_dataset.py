...
SAFETY — read before running anywhere but a local disposable database:
  ...
  * Refuses if the target database's own configured NAME looks like a
    ...
    "quorfix" — see docker-compose.yml / docker-compose.prod.yml — so a
    name-only gate would either always or never fire).
  * Refuses if the seed_demo organization (slug "quorfix-demo") or either
    ...
    database also used for demo or E2E data; point QUORFIX_DISPOSABLE_DATABASE
  ...
  * The --full (100,000-bug) profile additionally requires --full AND
    ...
    from QUORFIX_DISPOSABLE_DATABASE alone, or from either flag alone.
...
from apps.core.management.commands.seed_demo import DEMO_ORG_SLUG
...
# The demo org slug is imported from seed_demo, the single source of truth
# for it, so this guard can't silently go stale if it ever changes.
PROTECTED_ORG_SLUGS = frozenset({DEMO_ORG_SLUG, "bug-e2e-org", "analytics-e2e-org"})
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "Never inferred from DEBUG or from QUORFIX_DISPOSABLE_DATABASE alone.",
        )
        parser.add_argument(
    ...
    def _check_safety(self, options: dict) -> None:
        ...
        if not get_bool("QUORFIX_DISPOSABLE_DATABASE", False):
            raise CommandError(
                "generate_perf_dataset refuses to run without QUORFIX_DISPOSABLE_DATABASE=true "
        ...
        if protected:
            raise CommandError(
                ...
                "Point QUORFIX_DISPOSABLE_DATABASE at a genuinely separate, disposable database "
        ...
        if options["cleanup_existing_perf_data"] or options["full"]:
            if not options["confirm_disposable_database"]:
                ...
                raise CommandError(
                    ...
                    "never inferred from DEBUG, from QUORFIX_DISPOSABLE_DATABASE alone, or from "
