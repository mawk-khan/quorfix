...
SAFETY — read before running anywhere but a local disposable database:
  ...
  * Refuses if the seed_demo organization (current slug "quorfix-demo", or
    the pre-rename "bug-fixer-demo" — see seed_demo.LEGACY_DEMO_ORG_SLUG) or
    either E2E fixture organization (slugs "bug-e2e-org",
    "analytics-e2e-org") exists in the target database — this command must
    never run against a database also used for demo or E2E data; point
    BUGFIXER_DISPOSABLE_DATABASE at a genuinely separate database instead.
...
from apps.core.management.commands.seed_demo import DEMO_ORG_SLUG, LEGACY_DEMO_ORG_SLUG
...
# Both the current and pre-rename demo org slugs are protected (imported
# from seed_demo, the single source of truth for both — see that module's
# own LEGACY_DEMO_ORG_SLUG comment) so this guard can't silently go stale
# for a database seeded before the Quorfix rebrand.
PROTECTED_ORG_SLUGS = frozenset(
    {DEMO_ORG_SLUG, LEGACY_DEMO_ORG_SLUG, "bug-e2e-org", "analytics-e2e-org"}
)
